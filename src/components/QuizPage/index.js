...
const QuizPage = () => {
    ...
    var [endTime, setEndTime] = useState(today.setDate(today.getDate() + 7));
    ...
    const createQuiz = (e) => {
        axios.post(`${process.env.REACT_APP_SERVER}/class/quiz/create`, {
            quizName: quizName,
            openTime: openTime,
            endTime: endTime,
            quizUrl: quizUrl,
    ...
    const loadQuiz = () => {
        console.log(userId);
        console.log(classId);
        axios.get(`${process.env.REACT_APP_SERVER}/class/quiz/load`, {params: {
            userId: userId, 
            classId: classId
        },
        headers: {
